...
class AppStore extends Component {
  state = {activateTabsId: tabsList[0].tabId, text: ''}

  changeActivateTabsid = tabId => {
    this.setState({activateTabsId: tabId})
    // console.log('tab clicked')
  }

  categoryWiseList = () => {
    const {activateTabsId} = this.state
    const categoryLists = appsList.filter(
      each => each.category === activateTabsId,
    )
  ...
  render() {
    const {activateTabsId, text} = this.state
    const categoryList = this.categoryWiseList().filter(each =>
      each.appName.toLowerCase().includes(text.toLowerCase()),
    )
          ...
          <ul className="tabitem-card">
            {tabsList.map(each => (
              <TabItem
                eachList={each}
                key={each.tabId}
                isActivated={each.tabId === activateTabsId}
                changeActivateTabsid={this.changeActivateTabsid}
              />
            ))}
          </ul>
          <ul className="app-item-card">
            {categoryList.map(each => (
              <AppItem eachList={each} key={each.appId} />
            ))}
          </ul>
        </div>
      </div>
